...
//
// Every variant is .strict() — rejects unknown top-level keys (CCR-5 `.strict()` convention).
...
import { z } from 'zod';
import { SceneId, TokenId, ItemId, ActorId } from '../../branded-ids.js';
import { FOUNDRY_ID } from '../../primitives.js';

const keyId = z.string().min(1); // BRANDED-ID-EXEMPT:keyId — module-internal id, not a Foundry document id
...
export const getLockStateInput = z.object({
  ...
  documentId: FOUNDRY_ID.optional(),
...
export const createKeyInput = z.object({
  ...
  lockDocumentId: FOUNDRY_ID.optional(),
