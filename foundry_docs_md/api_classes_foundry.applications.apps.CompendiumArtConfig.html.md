...
## Hierarchy  
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.apps.CompendiumArtConfig)
...
static PARTS: {
    ...
    priorities: { id: string; template: string };
} = ...
```
...
**Returns**  
`Promise<{ buttons: { icon: string; label: string; type: string }[]; config: CompendiumArtDescriptor[] }>`
