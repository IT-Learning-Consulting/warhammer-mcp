# AdventureExporter | Foundry Virtual Tabletop - API Documentation - Version 13
...
```typescript
contentTree: Record<string, AdventureContentTreeNode> = {}
```
...
```typescript
static PARTS = {
    contents: { scrollable: string[], template: string },
    footer: { template: string },
    summary: { scrollable: string[], template: string },
...
**Returns:** Promise resolving to an extended `ApplicationRenderContext` with additional properties
...
_processSubmitData(
    ...
    submitData: any,
    options?: {},
): Promise<undefined | Readonly<Notification>>
```
...
Stage a document for addition to the Adventure. This adds the Document locally, the change  
is not yet submitted to the database.

**Parameters**  
- **document**: `any` — Some document to be added to the Adventure.
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<AdventureExporter>
```
...
**Parameters**  
- **options** (optional):  
  - Partial<ApplicationClosingOptions> — Options which modify how the application is closed.
...
**Parameters**  
- **document**: `ClientDocument` — The Document being removed from the Adventure.
...
**Parameters**  
- **options** (optional):  
  - `boolean` or `ApplicationRenderOptions & DocumentSheetRenderOptions` — Options which configure application rendering behavior. A boolean is interpreted as the `"force"` option.  
- **_options** (optional):  
  - `ApplicationRenderOptions & DocumentSheetRenderOptions` — Legacy options for backwards-compatibility with the original ApplicationV1#render signature.
