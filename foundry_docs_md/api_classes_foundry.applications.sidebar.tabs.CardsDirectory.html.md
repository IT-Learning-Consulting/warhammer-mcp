# CardsDirectory | Foundry Virtual Tabletop - API Documentation - Version 13
...
`options: Readonly<[DocumentDirectoryConfiguration](https://foundryvtt.com/api/types/foundry.DocumentDirectoryConfiguration.html)>`
...
`get documentClass(): [Constructor](https://foundryvtt.com/api/types/foundry.types.Constructor.html)<TDocument>`
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<CardsDirectory>
```
...
**Returns** `Promise<CardsDirectory>` - A Promise which resolves to the closed Application instance  
...
**Returns** `Promise<documents.Cards>`  
...
_createDroppedFolderDocuments(
    folder: documents.Folder,
    documents: object[] | documents.Cards[],
): Promise<void>
```
...
- **documents**: `object[] | documents.Cards[]` - The documents to create, or their indices.
...
- **options** (optional): `{ _skipDeprecation?: boolean } = {}`  
...
_organizeDroppedFoldersAndDocuments(
    ...
    documentsToCreate: object[] | documents.Cards[];
...
**Returns** `Promise<{ documentsToCreate: object[] | documents.Cards[]; foldersToCreate: documents.Folder[]; }>`  
