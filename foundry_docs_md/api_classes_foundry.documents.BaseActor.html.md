...
The Actor Document. Defines the DataSchema and common behaviors for an Actor which are shared between both client and server.
...
## Hierarchy  
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseActor) | Expand
...
```typescript
static DEFAULT_ICON: string = CONST.DEFAULT_TOKEN
```
...
Overrides [Document._initializeSource](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_initializeSource)
...
- **data**:  
  - `object` or  
  - `Document<object, DocumentConstructionContext>` or  
  - Array of either above (Optional)  
...
```typescript
static getDefaultArtwork(actorData: ActorData): { img: string; texture: { src: string } }
```
...
## Protected Static Methods (Batch Operations)
...
- [BaseActor class](https://foundryvtt.com/api/classes/foundry.documents.BaseActor.html)