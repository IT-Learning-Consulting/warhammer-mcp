...
- **options**: `any` = {}  
  The original enrichment options for cases where the Document embed content also contains text that must be enriched. (Optional)
...
Returns the data schema definition with:
...
- **options**: `{ events?: string[]; initial?: string[] }` = {} (Optional)  
  ...
  - **events**?: `string[]`  
    The event names to restrict to (Optional).

  - **initial**?: `string[]`  
    The initial set of events that should be default for the field (Optional).
