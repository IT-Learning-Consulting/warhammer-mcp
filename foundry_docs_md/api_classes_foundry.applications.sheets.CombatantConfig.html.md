# CombatantConfig | Foundry Virtual Tabletop - API Documentation - Version 13

**Class CombatantConfig**  
The Combatant configuration application.

[Mixes HandlebarsApplication](https://foundryvtt.com/api/modules/foundry.html#HandlebarsApplication)

[Hierarchy (View Summary, Expand)](https://foundryvtt.com/api/hierarchy.html#foundry.applications.sheets.CombatantConfig)  
...
`changeTab` method is called. It reports the active tab for each group, with a value of `null`  
...
(Inherited from HandlebarsApplicationMixin(DocumentSheetV2).DEFAULT_OPTIONS)
...
(Inherits and overrides HandlebarsApplicationMixin(DocumentSheetV2)._prepareContext)
...
Once ApplicationV1 is deprecated, this should switch from `_maxZ` to `ApplicationV2#maxZ`. Also, eliminate `ui.activeWindow` in favor of only `ApplicationV2#frontApp`.
...
- **options?**: `Partial<ApplicationClosingOptions>` — Options which modify how the application is closed (optional, default `{}`)

**Returns:**  
`Promise<CombatantConfig>` — A Promise which resolves to the closed Application instance
...
- **options?**: `boolean | (ApplicationRenderOptions & DocumentSheetRenderOptions)` — Options which configure application rendering behavior. A boolean is interpreted as the "force" option. Optional, default `{}`  
- **_options?**: `ApplicationRenderOptions & DocumentSheetRenderOptions` — Legacy options for backwards-compatibility with the original ApplicationV1#render signature. Optional, default `{}`

**Returns:**  
`Promise<CombatantConfig>` — A Promise which resolves to the rendered Application instance
...
- **submitOptions?**: `object` — Arbitrary options which are supported by and provided to the configured form submission handler. Optional, default `{}`
...
- **options?**: `{ animate?: boolean }` — Options to configure the toggling behavior (optional, default `{}`)  
...
- **options?**: Object (optional) — Additional options which affect ContextMenu construction  
...
- **updateData?**: `object` — Additional data passed in if this form is submitted manually which should be merged with prepared formData (optional)
...
- **options?**: `Partial<DatabaseUpdateOperation | DatabaseCreateOperation>` — Additional options altering the request (optional)
