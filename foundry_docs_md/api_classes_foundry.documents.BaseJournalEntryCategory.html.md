# BaseJournalEntryCategory | Foundry Virtual Tabletop - API Documentation - Version 13

An embedded Document that represents a category in a JournalEntry. Defines the  
DataSchema and common behaviors for a JournalEntryCategory which are shared between  
...
- [documents.JournalEntryCategory](https://foundryvtt.com/api/classes/foundry.documents.JournalEntryCategory.html)
...
const createdCompendium = await Actor.implementation.createDocuments(dataCompendium, {pack: "mymodule.mypack"});
```
...
const deletedCompendium = await Actor.implementation.deleteDocuments([actor.id], {pack: "mymodule.mypack"});
```
...
const actor = await pack.getDocument(documentId);
const updatedCompendium = await Actor.implementation.updateDocuments([{_id: actor.id, name: "New Name"}], {pack: "mymodule.mypack"});
```
