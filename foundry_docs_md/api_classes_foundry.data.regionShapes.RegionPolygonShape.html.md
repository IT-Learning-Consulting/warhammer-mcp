...
A polygon of a [foundry.documents.RegionDocument](https://foundryvtt.com/api/classes/foundry.documents.RegionDocument.html).
...
```typescript
new RegionPolygonShape(data: PolygonShapeData): RegionPolygonShape
```

**Parameters**

- **data**: `PolygonShapeData`  
  The polygon shape data.
...
```typescript
readonly clipperPaths: readonly (readonly IntPoint[])[]
```
...
```typescript
readonly clipperPolyTree: PolyTree
```
