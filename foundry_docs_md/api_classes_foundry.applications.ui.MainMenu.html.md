...
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.ui.MainMenu)
...
```typescript
new MainMenu(options?: Partial<ApplicationConfiguration>): MainMenu
```
...
static DEFAULT_OPTIONS: {
  actions: { menuItem: (...this: any, event: PointerEvent) => void };
  classes: string[];
  id: string;
  tag: string;
...
```typescript
static PARTS: { items: { template: string } } = ...
```
...
```typescript
_prepareContext(_options: any): Promise<{ items: Record<string, MainMenuItem> }>
```
...
**Returns**  
`Promise<{ items: Record<string, MainMenuItem> }>`
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<MainMenu>
```
