...
A simple shader whose purpose is to make the original texture's red channel the alpha channel, while still keeping channel information. Used in conjunction with the AlphaBlurFilterPass and Fog of War.
...
- **batchGeometry**  
  Type: `typeof BatchGeometry | { id: string; normalized: boolean; size: number; type: TYPES }[] = PIXI.BatchGeometry`  
...
- **defaultUniforms**  
  Type: `{ sampler: number; tintAlpha: number[] } = ...`  
  ...
  Inherited from [BaseSamplerShader.defaultUniforms](https://foundryvtt.com/api/classes/foundry.canvas.rendering.shaders.BaseSamplerShader.html#defaultuniforms)
...
### _packInterleavedGeometry (protected static)
...
### _preRenderBatch (protected static)
...
Pause or unpause this sampler. If set to true, the shader is disabled. Otherwise, it is enabled. Contrary to `enabled`, a shader might decide to refuse a pause, to continue rendering animations, for example.  
**Returns:** `boolean`  
...
The adjustments made inside fragment shaders.  
**Returns:** `string`  
...
A one-time initialization performed upon creation.  
...
### initializeBatchGeometry (static)
...
For more details, see the [FogSamplerShader API documentation](https://foundryvtt.com/api/classes/foundry.canvas.rendering.shaders.FogSamplerShader.html).