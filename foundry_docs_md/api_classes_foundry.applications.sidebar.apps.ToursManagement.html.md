# ToursManagement | Foundry Virtual Tabletop - API Documentation - Version 13
...
`options: Readonly<ApplicationConfiguration & CategoryBrowserConfiguration>`
...
`changeTab` method is called. Reports the active tab for each group, with a value of  
`null` indicating no tab is active. Subclasses may override this property to define default tabs for  
each group.
...
DEFAULT_OPTIONS: {
    actions: {
        ...
        reset: (event: PointerEvent, target: HTMLElement) => void | Promise<void>;
        resetDefaults: (
            event: PointerEvent,
            target: HTMLElement,
        ) => void | Promise<void>;
...
```typescript
protected _configureRenderParts(options: any): any
```
...
```typescript
protected _getTabsConfig(group: any): null | ApplicationTabsConfiguration
```
...
```typescript
protected _initializeApplicationOptions(options: any): ApplicationConfiguration
```
...
```typescript
protected _onRender(context: any, options: any): Promise<void>
```
...
```typescript
protected _sortCategories(a: any, b: any): any
```
...
```typescript
protected _tearDown(options: any): void
```
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<ToursManagement>
```
...
**Returns**  
`Promise<ToursManagement>` A Promise which resolves to the closed Application instance
...
```typescript
protected _canRender(options: HandlebarsRenderOptions): false | void
```
...
- **options?**: Additional options which affect ContextMenu construction
...
```typescript
protected _renderFrame(options: HandlebarsRenderOptions): Promise<HTMLElement>
```
...
For full context and additional details, see the [Foundry Virtual Tabletop API Documentation - Version 13](https://foundryvtt.com/api/classes/foundry.applications.sidebar.apps.ToursManagement.html).