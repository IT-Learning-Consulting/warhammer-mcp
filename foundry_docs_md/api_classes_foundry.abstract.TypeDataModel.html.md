# TypeDataModel | Foundry Virtual Tabletop - API Documentation - Version 13
...
- [DataModel](https://foundryvtt.com/api/classes/foundry.abstract.DataModel.html) _italic_
- **TypeDataModel**
- [RegionBehaviorType](https://foundryvtt.com/api/classes/foundry.data.regionBehaviors.RegionBehaviorType.html) _italic_
...
```typescript
new TypeDataModel(data?: {}, options?: {}): TypeDataModel
```
...
- **context**: `DataModelConstructionContext = {}` (Optional) - Context options passed to the data model constructor
...
- **options**: `any = {}` (Optional) - The original enrichment options for cases where the Document embed content also contains text that must be enriched.
...
- **source**: `boolean = true` (Optional) - Draw values from the underlying data source rather than transformed values.
