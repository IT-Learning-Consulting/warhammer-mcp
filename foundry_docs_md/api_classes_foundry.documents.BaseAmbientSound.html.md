# BaseAmbientSound | Foundry Virtual Tabletop - API Documentation - Version 13

The AmbientSound Document. Defines the DataSchema and common behaviors for an AmbientSound which are shared between both client and server.

Mixes:  
**AmbientSoundData**

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseAmbientSound), Expand)  
- _Document_ ([link](https://foundryvtt.com/api/classes/foundry.abstract.Document.html))  
- **BaseAmbientSound**  
- _AmbientSoundDocument_ ([link](https://foundryvtt.com/api/classes/foundry.documents.AmbientSoundDocument.html))
...
- **options?**: `{ invalid?: boolean; strict?: boolean } = {}` - Additional options which modify how embedded documents are retrieved  
  - Optional **invalid?**: `boolean` - Allow retrieving an invalid Embedded Document.  
  - Optional **strict?**: `boolean` - Throw an Error if the requested id does not exist. See Collection#get  
...
Get the value of a "flag" for this document. Flags represent key-value data used for flexible or arbitrary storage required by the core software, game systems, or user-created modules.
...
- **options?**: `{ exact?: boolean } = {}` - Additional options involved in the permission test  
  - Optional **exact?**: `boolean` - Require the exact permission level requested?  
...
**Parameters**  
- **data?**: `object | Document | (object | Document)[]` = `undefined` - Initial data used to create this Document, or a Document instance to persist.  
...
- **options?**: `{ embedded?: boolean }` = `{}` - Additional shimming options  
  - Optional **embedded?**: `boolean` - Apply shims to embedded models?  
...
Modifications to the requested deletions are performed by mutating the operation object. Use [updateSource](#updateSource).
...
For full API details and additional examples, visit the [BaseAmbientSound API Documentation](https://foundryvtt.com/api/classes/foundry.documents.BaseAmbientSound.html).