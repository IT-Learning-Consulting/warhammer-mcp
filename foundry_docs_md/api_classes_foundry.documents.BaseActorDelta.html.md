...
The ActorDelta Document. Defines the DataSchema and common behaviors for an ActorDelta which are shared between both client and server. ActorDeltas store a delta that can be applied to a particular Actor in order to produce a new Actor.

Mixes: `ActorDeltaData`

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseActorDelta), Expand)  
- [Document](https://foundryvtt.com/api/classes/foundry.abstract.Document.html)  
- **BaseActorDelta**  
- [documents.ActorDelta](https://foundryvtt.com/api/classes/foundry.documents.ActorDelta.html)
...
Returns: Object containing field and joint validation failures  
...
The canonical name of this Document type, e.g. `"Actor"`.  
Returns: `string`  
...
Returns: Readonly Record of the hierarchy  
...
- or `Promise<Document<object, DocumentConstructionContext>>`  
...
Get the value of a "flag" for this document. Flags are key-value data used for flexible or arbitrary data storage by core, systems, or modules.
...
Assign a "flag" to this document. Flags represent key-value data which can be used to store flexible or arbitrary data required by core software, game systems, or user-created modules.

Flags should be set using a scope which provides a namespace for the flag to help prevent collisions.

- Core software uses the `"core"` scope.
- Game systems or modules should use their canonical module name.
- Individual worlds should use the `"world"` scope.

Setting a flag value to `null` deletes that flag.
...
Overrides [Document.toObject](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#toobject)
...
Overrides [Document.updateSource](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#updatesource)
...
- **delta**: `ActorDelta` — The ActorDelta.
- **baseActor**: `BaseActor` — The base Actor.
- **context**: `object` = `{}` (optional) — Context to supply to synthetic Actor instantiation.

**Returns:** `null | BaseActor`
...
- **data**: `object | Document<object, DocumentConstructionContext> | (object | Document<object, DocumentConstructionContext>)[]` (optional) — Initial data used to create this Document, or a Document instance to persist.
- **operation**: `Partial<Omit<DatabaseCreateOperation, "data">>` = `{}` (optional) — Parameters of the creation operation

**Returns:** Promise resolving to the created Document instance(s).
...
- **data**: `(object | Document<object, DocumentConstructionContext>)[]` = `[]` (optional) — An array of data objects or existing Documents to persist.
- **operation**: `Partial<Omit<DatabaseCreateOperation, "data">>` = `{}` (optional) — Parameters of the requested creation operation

**Returns:** Promise resolving to an array of created Document instances.
