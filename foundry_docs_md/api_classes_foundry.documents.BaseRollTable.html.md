...
The RollTable Document. Defines the DataSchema and common behaviors for a RollTable which are shared between both client and server.

Mixes:  
RollTableData

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseRollTable), Expand):  
- [Document](https://foundryvtt.com/api/classes/foundry.abstract.Document.html)
- **BaseRollTable**
- [documents.RollTable](https://foundryvtt.com/api/classes/foundry.documents.RollTable.html)
...
```typescript
delete(operation?: Partial<Omit<DatabaseDeleteOperation, "ids">>): Promise<undefined | Document<object, DocumentConstructionContext>>
```
...
Get the value of a "flag" for this document. Flags represent key-value data which can be used by the core software, game systems, or user-created modules.
...
This method returns the value recorded in Document ownership, regardless of the User's role, e.g. a GAMEMASTER user might still return a result of NONE if they are not explicitly denoted as having a level.
...
Yields embedded Document instances.
...
Validate the data contained in the document to check for type and content. If changes are provided, missing types are added before cleaning and validation. This mutates the provided changes. Throws an error if data within the document is not valid.
...
**Returns**  
`boolean` — Whether the data source or proposed change is valid. Always returns boolean if validation is non-strict.

**Throws**  
An error if validation is strict and a failure occurs.
...
- **data**: (Optional)  
  ...
  Initial data used to create the Document(s), or a Document instance to persist.
...
- **operation**: `Partial<Omit<DatabaseCreateOperation, "data">>` = `{}` (Optional)  
  Requested creation operation parameters
...
**Returns**  
An object mapping schema fields to their definitions.
