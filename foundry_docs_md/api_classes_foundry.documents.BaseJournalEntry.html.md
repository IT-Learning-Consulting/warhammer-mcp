# BaseJournalEntry | Foundry Virtual Tabletop - API Documentation - Version 13

The JournalEntry Document. Defines the DataSchema and common behaviors for a JournalEntry which are shared between both client and server.

Mixes: JournalEntryData

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseJournalEntry), Expand)

- [Document](https://foundryvtt.com/api/classes/foundry.abstract.Document.html) (abstract)
- **BaseJournalEntry**
- [documents.JournalEntry](https://foundryvtt.com/api/classes/foundry.documents.JournalEntry.html)
...
*Inherited from [Document.schema (static)](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#schema)*
...
**Returns**:  
Either `Document` or `Promise<Document>` The cloned Document instance
...
- data: `object | Document | array` (optional)  
...
const createdActorItem = await Item.implementation.create(data, {parent: actor});

const createdPackItem = await Item.implementation.create(data, {pack: "mymodule.mypack"});
```
...
const createdEmbedded = await Item.implementation.createDocuments(itemData, {parent: actor});

const compendiumData = [{name: "Compendium Actor", type: "character", img: "path/to/profile.jpg"}];
const createdCompendium = await Actor.implementation.createDocuments(compendiumData, {pack: "mymodule.mypack"});
```
...
const deletedCompendium = await Actor.implementation.deleteDocuments([actorFromPack.id], {pack: "mymodule.mypack"});
```
...
const actorFromPack = await pack.getDocument(documentId);
const updatedCompendium = await Actor.implementation.updateDocuments([{_id: actorFromPack.id, name: "New Name"}], {pack: "mymodule.mypack"});
```
