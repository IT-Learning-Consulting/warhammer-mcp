...
**Hierarchy:**  
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.sheets.RegionBehaviorConfig)
...
- **DEFAULT_OPTIONS**:  
  ```typescript
  {
    ...
    window: { contentClasses: string[]; icon: undefined };
  } = ...
  ```
  _Inherited from HandlebarsApplicationMixin(DocumentSheetV2).DEFAULT_OPTIONS_
...
- **id**: `string`  
  ...
  internal ID used by this application. This getter should not be overridden by subclasses, which should  
  instead configure the ID in `DEFAULT_OPTIONS` or by defining a `uniqueId` during  
...
- **window**:
  ...
  _Inherited from HandlebarsApplicationMixin(DocumentSheetV2).window_
...
**Returns**  
`Promise<ApplicationRenderContext & { document: ClientDocument; editable: boolean; fields: any; rootId: string; source: any; user: null | documents.User; } & { buttons: FormFooterButton[]; fields: FormNode[]; region: ClientDocument; }>`
...
_Inherited from HandlebarsApplicationMixin(DocumentSheetV2)._renderHTML_
...
**Returns**  
`Promise<RegionBehaviorConfig>` — A Promise which resolves to the closed Application instance  
...
**Returns**  
`FormFooterButton[]`  
Inherited from HandlebarsApplicationMixin(DocumentSheetV2)._getButtons
...
**Returns**  
`FormNode[]`  
Inherited from HandlebarsApplicationMixin(DocumentSheetV2)._getFields
...
- **target**: `HTMLElement` — The capturing HTML element which defined a `[data-action]` attribute
...
- **updateData?**: `object` — Additional data passed in if this form is submitted manually which should be merged with  
  prepared formData.
