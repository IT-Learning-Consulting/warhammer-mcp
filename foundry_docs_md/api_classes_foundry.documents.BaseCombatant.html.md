# BaseCombatant | Foundry Virtual Tabletop - API Documentation - Version 13

The Combatant Document. Defines the DataSchema and common behaviors for a Combatant which are shared between both client and server.

**Mixes**  
CombatantData

**Hierarchy** ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseCombatant), Expand)  
- [Document](https://foundryvtt.com/api/classes/foundry.abstract.Document.html)  
- **BaseCombatant**  
- [documents.Combatant](https://foundryvtt.com/api/classes/foundry.documents.Combatant.html)
...
**Returns**  
`any` - A numeric permission level from CONST.DOCUMENT_OWNERSHIP_LEVELS
...
- `undefined`, or  
- a `Document<object, DocumentConstructionContext>`, or  
- an array of such Document instances
...
const createdCompendiumItem = await Item.implementation.create(data, {pack: "mymodule.mypack"});
```
...
const createdEmbedded = await Item.implementation.createDocuments(dataItems, {parent: actor});
...
**Returns**  
An object containing field definitions as listed above.
...
const deletedInPack = await Actor.implementation.deleteDocuments([actorInPack.id], {pack: "mymodule.mypack"});
```
...
For full details and additional methods, see the [Foundry Virtual Tabletop API Documentation for BaseCombatant](https://foundryvtt.com/api/classes/foundry.documents.BaseCombatant.html).