...
See [View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.sidebar.tabs.Settings)
...
```typescript
new Settings(options?: Partial<ApplicationConfiguration>): Settings
```
...
- **DEFAULT_OPTIONS**
  ...
  DEFAULT_OPTIONS: {
      actions: {
          notifyUpdate: (event: any) => Promise<void>;
          openApp: (...this: any, event: any) => Promise<void>;
      };
      window: { title: string };
  } = ...
  ```
...
Prepare the rendering context for the Application.
...
`Promise` resolving to the extended `ApplicationRenderContext`
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<Settings>
```
...
`Promise<Settings>` — A Promise which resolves to the closed Application instance.
...
- [System](https://foundryvtt.com/api/classes/foundry.packages.System.html)