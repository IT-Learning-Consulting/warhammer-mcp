...
A subclass of [foundry.data.fields.StringField](https://foundryvtt.com/api/classes/foundry.data.fields.StringField.html) which supports referencing some other Document by its UUID. This field may not be blank, but may be null to indicate that no UUID is referenced.

See the [Hierarchy View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.data.fields.DocumentUUIDField)
...
Overrides [StringField._defaults](https://foundryvtt.com/api/classes/foundry.data.fields.StringField.html#_defaults)
...
```typescript
_toInput(config: any): HTMLDocumentTagsElement
```
...
- **groupConfig**?: `FormGroupConfig` = `{}` — Configuration options passed to the wrapping form-group
- **inputConfig**?: `FormInputConfig` = `{}` — Input element configuration options passed to DataField#toInput
...
- **options**?: `object` — Additional options for how the field is cleaned.
