...
The data model for a behavior that teleports Tokens that enter the Region to a preset destination Region.
...
- **Inherited from:** [_source property of RegionBehaviorType](https://foundryvtt.com/api/classes/foundry.data.regionBehaviors.RegionBehaviorType.html#_source)

### events

- **Type:** `Set<string>`  
- **Description:** The events that are handled by the behavior.  
- **Inherited from:** [events property of RegionBehaviorType](https://foundryvtt.com/api/classes/foundry.data.regionBehaviors.RegionBehaviorType.html#events)
...
- **Inherited from:** [parent property of RegionBehaviorType](https://foundryvtt.com/api/classes/foundry.data.regionBehaviors.RegionBehaviorType.html#parent)

### Static events

```typescript
events: {
    tokenMoveIn: (...this: any, event: RegionMoveInEvent) => Promise<void>;
} = ...
```
...
- Returns a convenience reference to the [RegionBehavior](https://foundryvtt.com/api/classes/foundry.documents.RegionBehavior.html) which contains this behavior sub-type.  
...
- **Description:** A convenience reference to the [RegionDocument](https://foundryvtt.com/api/classes/foundry.documents.RegionDocument.html) which contains this behavior sub-type.  
...
- **Description:** A convenience reference to the [Scene](https://foundryvtt.com/api/classes/foundry.documents.Scene.html) which contains this behavior sub-type.  
...
- **options** (optional): `any` = `{}` - The original enrichment options for cases where the Document embed content also contains text that must be enriched.
...
- **token**: `TokenDocument` - The token being or about to be moved within the region of this behavior  
- **segment**: Movement segment data including shape, height, width, action, and a preview flag  

**Returns:**  
Array of `TerrainEffect` that apply to this token's movement
...
- **event**: `RegionEvent` - The Region event  
...
- **options** (optional): `{ events?: string[]; initial?: string[] }` = `{}` - Options which configure how the events field is declared  

**Options:**

- **events** (optional): `string[]` - The event names to restrict to.  
- **initial** (optional): `string[]` - The initial set of events that should be default for the field  
