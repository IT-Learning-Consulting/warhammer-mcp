...
**Hierarchy:**  
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.ui.RegionLegend)
...
```typescript
new RegionLegend(options?: Partial<ApplicationConfiguration>): RegionLegend
```
...
DEFAULT_OPTIONS: {
    actions: {
        config: (event: PointerEvent) => void;
        control: (event: PointerEvent) => void;
        create: (event: PointerEvent) => Promise<void>;
        delete: (event: PointerEvent) => Promise<void>;
        lock: (event: PointerEvent) => Promise<void>;
    };
    id: string;
    position: { width: number };
    tag: string;
    window: { icon: string; minimizable: boolean; title: string };
} = ...
```
...
`PARTS: { list: { id: string; scrollable: string[]; template: string } } = ...`
...
```typescript
_canRender(options: any): undefined | false
```
...
_prepareContext(
    ...
    elevation: { bottom: string | number; top: string | number };
    regions: any;
}>
```
...
**Returns**  
`Promise<{ elevation: { bottom: string | number; top: string | number }; regions: any }>`  
...
Overrides `HandlebarsApplicationMixin(ApplicationV2)._renderFrame`
...
```typescript
close(options?: {}): Promise<RegionLegend>
```
...
**Returns**  
`Generator<ApplicationHeaderControlsEntry, any, any>` — Yields
...
- [RegionLegend class API](https://foundryvtt.com/api/classes/foundry.applications.ui.RegionLegend.html)