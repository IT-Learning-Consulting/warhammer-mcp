# CardDeckConfig | Foundry Virtual Tabletop - API Documentation - Version 13
...
```typescript
PARTS: {
    cards: { scrollable: string[]; template: string };
...
**Returns:** Generator of `ApplicationHeaderControlsEntry`
...
**Returns:** Promise of the extended `ApplicationRenderContext`
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<CardDeckConfig>
```
...
**Returns:** `Promise<CardDeckConfig>` — A Promise which resolves to the closed Application instance
...
**Returns:** `Promise<CardDeckConfig>` — A Promise which resolves to the rendered Application instance
...
- **sortMode?**: `"standard" | "shuffled"` — Default value assumed
...
- [FormFooterButton](https://foundryvtt.com/api/interfaces/foundry.applications.types.FormFooterButton.html)
