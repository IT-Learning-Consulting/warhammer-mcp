# BaseFolder | Foundry Virtual Tabletop - API Documentation - Version 13

The Folder Document. Defines the DataSchema and common behaviors for a Folder which are shared between both client and server.

Mixes:  
- FolderData

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseFolder), Expand):

- [Document](https://foundryvtt.com/api/classes/foundry.abstract.Document.html) (abstract base class)  
- **BaseFolder**  
...
- **data**: `Partial<FolderData>` = {}  
  Optional initial data used to construct the document.
...
- **options**:  
  ...
  - **strict**?: `boolean` (Optional) - Throw an Error if the requested id does not exist. (See Collection#get)
...
- **options**: `{ exact?: boolean }` = {} (Optional)  
  ...
  - **exact**?: `boolean` (Optional) Require the exact permission level requested?
...
Initialize the instance by copying data from the source object to instance attributes. This mirrors the workflow of [SchemaField#initialize](https://foundryvtt.com/api/classes/foundry.data.fields.SchemaField.html#initialize) but with some added functionality.
...
Returns the initialization order generator.
...
```typescript
static get(documentId: any, options?: {}): null | Document<object, DocumentConstructionContext>
```

Overrides [Document.get](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#get)
...
- **options**: `{ embedded?: boolean }` = {} (Optional)  
  ...
  - **embedded**?: `boolean` (Optional) - Apply shims to embedded models?
