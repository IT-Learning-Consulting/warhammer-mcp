# BaseFogExploration | Foundry Virtual Tabletop - API Documentation - Version 13

The FogExploration Document. Defines the DataSchema and common behaviors for a FogExploration which are shared between both client and server.

**Mixes**  
FogExplorationData

**Hierarchy** ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseFogExploration), Expand)  
- [Document](https://foundryvtt.com/api/classes/foundry.abstract.Document.html) (abstract)  
- **BaseFogExploration**  
- [documents.FogExploration](https://foundryvtt.com/api/classes/foundry.documents.FogExploration.html)
...
new BaseFogExploration(
    data?: Partial<FogExplorationData>, 
...
- **Parameters**
  ...
  - **options?**: `DocumentConstructionContext` = `{}`
    ...
    Optional. Context and data validation options which affect initial model construction.
...
- Returns: Object with `fields` and `joint` validation failures nullable.  
...
- Overrides [Document._preUpdate](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_preupdate)
...
- **Returns**: `Document` instance or Promise resolving to a Document  
...
- **Returns**: `Document` instance or undefined  
...
- Compendium content ignores the ownership field in favor of User role-based ownership.  
...
- **Returns**: `Promise<undefined | Document>` - The updated Document instance, or undefined if not updated  
...
- Returns a generator for initialization order.  
...
- Generally used to verify whether a User should be presented with the option to create Documents of this type in the UI.  
- **Parameters**  
  - user: `BaseUser` - The User being tested  
...
- **Parameters**  
  - data? (optional): Initial data used to create this Document, or a Document instance to persist.  
  - operation? (optional): `Partial<Omit<DatabaseCreateOperation, "data">>` = `{}` - Parameters of the creation operation  
- **Returns**: `Promise` resolving to the created Document instance(s) or undefined  
- Examples:  
...
- **Parameters**  
  - data? (optional): Array of data objects or existing Documents to persist. Default `[]`.  
...
static defineSchema(): {
    ...
    explored: FilePathField;
    flags: DocumentFlagsField;
    positions: ObjectField;
...
- **Returns**: Object describing the fields and their types  
...
- **Returns**: `null | Document` instance or `null`  
...
```typescript
protected static async _onCreateOperation(
...
- **Parameters**  
  - documents: Array of created Document instances  
...
```typescript
protected static async _onDeleteOperation(
    documents: Document<object, DocumentConstructionContext>[],
    operation: DatabaseDeleteOperation,
    user: BaseUser,
): Promise<void>
...
- **Parameters**  
  - documents: Array of deleted Document instances  
...
```typescript
protected static async _onUpdateOperation(
    documents: Document<object, DocumentConstructionContext>[],
    operation: DatabaseUpdateOperation,
    user: BaseUser,
): Promise<void>
...
- **Parameters**  
  - documents: Array of updated Document instances  
...
```typescript
protected static async _preCreateOperation(
    documents: Document<object, DocumentConstructionContext>[],
    operation: DatabaseCreateOperation,
    user: BaseUser,
...
```typescript
protected static async _preDeleteOperation(
    documents: Document<object, DocumentConstructionContext>[],
    operation: DatabaseDeleteOperation,
    user: BaseUser,
): Promise<boolean | void>
...
```typescript
protected static async _preUpdateOperation(
    documents: Document<object, DocumentConstructionContext>[],
    operation: DatabaseUpdateOperation,
    user: BaseUser,
): Promise<boolean | void>
