...
Detection mode that see ALL creatures (no blockers). If not constrained by walls, see everything within the range.

[Foundry Virtual Tabletop - API Documentation - Version 13](https://foundryvtt.com/api/modules.html) / [foundry](https://foundryvtt.com/api/modules/foundry.html) / [canvas /](https://foundryvtt.com/api/modules/foundry.canvas.html) [perception](https://foundryvtt.com/api/modules/foundry.canvas.perception.html) /  
...
Returns: An object containing `fields` and `joint` validation failures or nulls.  
...
**Returns**: Migrated and cleaned source data which will be stored to the model instance, which is the same object as the `data` argument.  
...
```typescript
static getDetectionFilter(): Filter
```
...
**Returns**: Generator yielding `[string, DataField]` pairs.  
