# CardsConfig | Foundry Virtual Tabletop - API Documentation - Version 13
...
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.sheets.CardsConfig)
...
DEFAULT_OPTIONS: {
    actions: {
        ...
        pass: (event: PointerEvent, target: HTMLElement) => void | Promise<void>;
...
- **options.event?**: `Event` - An interaction event which caused the tab change, if any
- **options.force?**: `boolean` - Force changing the tab even if the new tab is already active
- **options.navElement?**: `HTMLElement` - An explicit navigation element being modified
- **options.updatePosition?**: `boolean` - Update application position after changing the tab?
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<CardsConfig>
```
...
- **submitOptions?**: `object` - Arbitrary options which are supported by and provided to the configured form submission  
  handler.
