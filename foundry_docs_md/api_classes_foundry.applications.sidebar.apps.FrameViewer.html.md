...
**Hierarchy:** [View Summary][hierarchy-link], Expand
...
**Type:** `Readonly<ApplicationConfiguration & FrameViewerConfiguration>`
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<FrameViewer>
```
...
- **options?**: `boolean | ApplicationRenderOptions = {}` — Options which configure application rendering behavior. A boolean is interpreted as the "force" option.
- **_options?**: `ApplicationRenderOptions = {}` — Legacy options for backwards-compatibility with the original ApplicationV1#render signature.
...
**Returns:** Generator yielding each class in the inheritance chain.
