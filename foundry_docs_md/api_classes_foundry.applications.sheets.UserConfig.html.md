# UserConfig | Foundry Virtual Tabletop - API Documentation - Version 13
...
Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.sheets.UserConfig))  
...
```typescript
{
  actions: { releaseCharacter: (event: PointerEvent) => void };
  classes: string[];
...
```typescript
{
  form: { id: string; template: string }
} = ...
```
...
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<UserConfig>
```
