# DrawingsLayer | Foundry Virtual Tabletop - API Documentation - Version 13

The `DrawingsLayer` subclass of `PlaceablesLayer`. This layer implements a container for drawings.
...
```typescript
graphics: Collection<string, canvas.placeables.Drawing> = ...
```
...
```typescript
DEFAULT_CONFIG_SETTING: string = "defaultDrawingConfig"
```
...
- `options`: `object` — Options passed to the control method of each object (default: `{}`)
...
- `options` (optional): `object` — Options which configure how the layer is drawn (default: `{}`)
...
- `options`: `object` — Options passed to the release method of each object (default: `{}`)
...
- `options` (optional): `object` — Options which configure how the layer is deconstructed (default: `{}`)
...
- `condition` (optional): `null | Function` — A function which tests whether to target each object. Default is `null`.
- `options` (optional): `object` — Additional options passed to `Document.update`. Default is `{}`.
...
- `origin`: `Point` — The initial coordinate.
...
For more information or related classes:
