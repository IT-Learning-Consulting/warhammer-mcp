# DetectionModeInvisibility | Foundry Virtual Tabletop - API Documentation - Version 13

Detection mode that see invisible creatures. This detection mode allows the source to:  
- See/Detect the invisible target as if visible.  
- The "See" version needs sight and is affected by blindness.

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.canvas.perception.DetectionModeInvisibility))  
...
Inherited from [DetectionMode.constructor](https://foundryvtt.com/api/classes/foundry.canvas.perception.DetectionMode.html#constructor)
...
**Returns**  
`Readonly<{ MOVE: number; OTHER: number; SIGHT: number; SOUND: number; }>`
