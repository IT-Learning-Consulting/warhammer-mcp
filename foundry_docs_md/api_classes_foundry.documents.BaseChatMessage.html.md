...
The ChatMessage Document. Defines the DataSchema and common behaviors for a ChatMessage which are shared between both client and server.

Mixes:  
**ChatMessageData**

Hierarchy ([View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.documents.BaseChatMessage), Expand):  
- *Document*  
- **BaseChatMessage**  
...
**Returns**  
| Document instance or Promise resolving to a Document instance
...
**Returns**  
`DocumentOwnershipNumber` (numeric permission level from [CONST.DOCUMENT_OWNERSHIP_LEVELS](https://foundryvtt.com/api/variables/CONST.DOCUMENT_OWNERSHIP_LEVELS.html))
...
- **options**: `{ exact?: boolean } = {}`  
  Optional. Additional options involved in the permission test
  - **exact?**: `boolean`  
...
Modifications to the pending Document instance must be performed using [updateSource](api_classes_foundry.documents.BaseChatMessage.html#20).
...
- **data**:  
  Optional. Initial data used to create this Document, or a Document instance to persist.  
  Types: object, Document, or array of either
- **operation**: `Partial<Omit<DatabaseCreateOperation, "data">>` = `{}`  
...
- **data**: `(object | Document<object, DocumentConstructionContext>)[]` = `[]`  
  Optional. An array of data objects or existing Documents to persist.
- **operation**: `Partial<Omit<DatabaseCreateOperation, "data">>` = `{}`  
  Optional. Parameters of the requested creation operation
...
- **ids**: `string[]` = `[]`  
  Optional. An array of string ids for the documents to be deleted
- **operation**: `Partial<Omit<DatabaseDeleteOperation, "ids">>` = `{}`  
  Optional. Parameters of the database deletion operation
...
- **updates**: `object[]` = `[]`  
  An array of differential data objects, each used to update a single Document
- **operation**: `Partial<Omit<DatabaseUpdateOperation, "updates">>` = `{}`  
  Optional. Parameters of the database update operation
...
*This batch-wise workflow occurs after individual* [_onCreate](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_oncreate) *workflows.*
...
*This batch-wise workflow occurs after individual* [_onDelete](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_ondelete) *workflows.*
...
*This batch-wise workflow occurs after individual* [_onUpdate](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_onupdate) *workflows.*
...
*This batch-wise workflow occurs after individual* [_preCreate](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_precreate) *workflows and provides a final pre-flight check before a database operation occurs.*

Modifications to pending documents must mutate the documents array or alter individual document instances using [updateSource](api_classes_foundry.documents.BaseChatMessage.html#20).
...
*This batch-wise workflow occurs after individual* [_preDelete](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_predelete) *workflows and provides a final pre-flight check before a database operation occurs.*

Modifications to the requested deletions are performed by mutating the operation object or using [updateSource](api_classes_foundry.documents.BaseChatMessage.html#20).
...
*This batch-wise workflow occurs after individual* [_preUpdate](https://foundryvtt.com/api/classes/foundry.abstract.Document.html#_preupdate) *workflows and provides a final pre-flight check before a database operation occurs.*
