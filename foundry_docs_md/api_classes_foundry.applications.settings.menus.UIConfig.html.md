...
**Hierarchy:**  
[View Summary](https://foundryvtt.com/api/hierarchy.html#foundry.applications.settings.menus.UIConfig)
...
If this Application uses tabbed navigation groups, this mapping updates whenever the `changeTab` method is called. Reports the active tab for each group, with a value of `null` indicating no tab is active. Subclasses may override this property to define default tabs for each group.  
...
{
  actions: {
    reset: (...this: any, event: InputEvent) => Promise<void>
  };
  form: {
...
**Returns:** `Promise` of an object containing:  
- **buttons**: array of action objects  
- **fields**: [`DataSchema`](https://foundryvtt.com/api/types/foundry.abstract.types.DataSchema.html)  
- **setting**: [`GameUIConfiguration`](https://foundryvtt.com/api/interfaces/foundry.GameUIConfiguration.html)
...
- **options?**: Object with optional property:  
...
Once ApplicationV1 is deprecated, switch from `_maxZ` to `ApplicationV2#maxZ`. Also eliminate `ui.activeWindow` in favor of only `ApplicationV2#frontApp`.  
...
### close
```typescript
close(options?: Partial<ApplicationClosingOptions>): Promise<UIConfig>
```
...
**Returns:** `Promise<UIConfig>` — A Promise which resolves to the closed Application instance.
...
```
Render the Application, creating its HTMLElement and replacing its innerHTML. Add it to the DOM if not currently rendered and rendering is forced. Otherwise, re-render its contents.  
Inherited from: `HandlebarsApplicationMixin(ApplicationV2).render`

**Parameters**  
- **options?**: `boolean | ApplicationRenderOptions` — Options which configure rendering behavior. A boolean is interpreted as the `"force"` option.  
- **_options?**: `ApplicationRenderOptions` — Legacy options for backwards-compatibility.

**Returns:** `Promise<UIConfig>` — A Promise which resolves to the rendered Application instance.
...
```
Update the Application element position using provided data merged with the prior position.  
...
**Parameters**  
- **expanded?**: `boolean` — Set the controls visibility to a specific state. Otherwise, toggles from current state.  
- **options?**: `{ animate?: boolean }` — Options to configure toggling behavior.

**Returns:** `Promise<void>` — Resolves once the control expansion animation is complete.
...
**Parameters**  
- **handler**: `() => ContextMenuEntry[]` — A handler function providing initial context options  
- **selector**: `string` — A CSS selector to which the ContextMenu will be bound  
- **options?**:  
  - **container?**: `HTMLElement` — A parent HTMLElement containing the selector target  
...
```
Iterate over header control buttons, filtering for controls visible for the current client.  
...
```
Initialize configuration options for the Application instance. Array-based options are concatenated, inner objects are merged, and otherwise subclass properties replace those defined by a parent.  
...
```
Insert the application HTML element into the DOM. Subclasses may override to customize insertion.  
...
```
A generic event handler for action clicks extendable by subclasses. Action handlers defined in `DEFAULT_OPTIONS` are called first. This method is only called for actions without a defined handler.  
Inherited from: `HandlebarsApplicationMixin(ApplicationV2)._onClickAction`

**Parameters**  
- **event**: `PointerEvent` — The originating click event  
- **target**: `HTMLElement` — The HTML element defining a `[data-action]`
...
```
Handle submission for an Application which uses a form element.  
...
```
Replace the HTML of the application with the result provided by the rendering backend. An Application subclass should implement this method for the Application to be renderable.  
...
```
Translate a requested application position into a resolved allowed position for the Application. Subclasses may override for advanced positioning behavior.  
...
*For full API details, visit the official [Foundry Virtual Tabletop API Documentation](https://foundryvtt.com/api/classes/foundry.applications.settings.menus.UIConfig.html).*