...
export class TemplateTool extends BaseTool {
  ...
  getRegistration(): Array<{ name: string; handler: (args: any) => Promise<any> }> {
    return [
        { name: 'template', handler: (args: any) => this.execute(args) },
    ];
  }
...
- list: {action:"list", sceneId:"abc", hidden:false}`,
        inputSchema: {
          ...
          properties: {
            ...
            borderColor: { type: 'string', description: '[create/update.changes] Border color as a 6-hex #RRGGBB string (same enforced constraint as fillColor).' },
            ...
            hidden: { type: 'boolean', description: '[create/update.changes] Hide the template from players. Also accepted on `list` as a boolean filter (BUG-511).' },
