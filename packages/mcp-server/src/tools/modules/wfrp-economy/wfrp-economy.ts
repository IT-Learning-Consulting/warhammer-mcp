...
//
// 52 actions across 14 idioms (stand-up-an-economy, open-a-bank-account, run-a-transaction, loan-cycle,
// investment-cycle, property-management, wallet-quick-adjust, audit-the-ledger, unified-ledger,
// levy-and-burn, banking-and-income, legitimate-business-enterprises, enterprise-ownership-and-debt,
// levy-groups). Anchors:
...
For create-economy, embed a stock to enable the buy/sell round-trip: stocks:[{ name, symbol, currentPrice, availableShares }]. Example: { action: "deposit", economyId: "abc", accountId: "def", amountBp: 240 }`,
        inputSchema: {
          ...
          properties: {
            ...
            ownerShares: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { actorId: { type: 'string' }, sharePct: { type: 'number' } }, required: ['actorId', 'sharePct'] }, description: 'set-enterprise-owners (required): the new owners list — [{ actorId, sharePct }], sharePct integers MUST sum to exactly 100.' },
