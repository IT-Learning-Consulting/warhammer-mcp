...
function formatResult(d: WfrpEconomyResult): string {
  ...
  switch (d.action) {
    ...
    case 'delete-venture':
      ...
      return `${p}.delete-venture: venture "${d.name}" (${d.ventureId}) removed from the ledger; transaction history retained.${d.writtenOffBp > 0 ? ` ${d.writtenOffBp} BP could not be paid to any holder and was written off — no coin moved to a wallet.` : ''}`;
    case 'venture-event':
