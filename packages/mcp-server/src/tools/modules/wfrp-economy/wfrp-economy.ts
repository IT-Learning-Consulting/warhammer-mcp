...
//
// 93 actions across 17 idioms (stand-up-an-economy, open-a-bank-account, run-a-transaction, loan-cycle,
...
// set-venture-standing; systemic_bug_class_prevention v2 Phase 2 (BUG-821(c)/ADR-16): +1 action —
// cleanup-orphan-ventures, GM-only confirm-gated one-time removal of the pre-existing zero-resolving-
// link orphan deeds], trading [Phase 7f: +20 actions, trading-* prefix; post-7f: +3 actions —
...
// BUG-822: handledBy is projected in `data` by the foundry-module handler (commit 23d1e7c) but
// was never surfaced in the rendered text — F03 (this file's own convention) requires the
// formatter to emit every field the handler returns, since an MCP client/eval grader can only
// observe content[].text, never structuredContent (PF-013).
function formatHandledBy(handledBy: Array<{ role: string; name?: string | null }> | undefined): string {
  if (!handledBy || handledBy.length === 0) return '';
  const entries = handledBy.map((h) => (h.name ? `${h.role} (${h.name})` : h.role));
  return `, handled by: ${entries.join(', ')}`;
}
...
function formatResult(d: WfrpEconomyResult): string {
  ...
  switch (d.action) {
    ...
    case 'create-venture':
      return `${p}.create-venture: "${d.name}" (${d.type}, ${d.ventureId}) — status ${d.status}, standing ${d.standing}${formatHandledBy(d.handledBy)}.`;
    case 'get-venture':
      // BUG-839: notices/deedDateText are promised by this tool's description but were never emitted
      // into content[].text (structuredContent-only) — the same emit-every-field gap BUG-822 fixed for
      // handledBy. Empty-safe suffixes mirroring formatHandledBy's style.
      return `${p}.get-venture: "${d.name}" (${d.ventureId}) — ${d.type}, ${d.status}/${d.standing}${d.readyToLaunch ? ' (ready to launch)' : ''}, ${d.partsSubscribed}/${d.partsTotal} Parts @ ${d.priceBp} BP, escrow ${d.escrowBp} BP (capital ${d.capitalBp} BP + ${Math.max(0, d.escrowBp - d.capitalBp)} BP above the line), quiet cycles ${d.quietCycles}, ${d.holders.length} holder(s), ${d.queuedTransfers.length} queued transfer(s)${d.badges.length ? `, badges: ${d.badges.join(', ')}` : ''}${formatHandledBy(d.handledBy)}${d.deedDateText ? `, dated ${d.deedDateText}` : ''}${d.notices.length ? `, notices: ${d.notices.join('; ')}` : ''}.`;
    case 'list-ventures':
      // M7 projection MUST reach the rendered text, not just structuredContent — an MCP client (and the
      // eval grader) observes content[].text only. Mirrors get-venture's readyToLaunch/quietCycles.
      return `${p}.list-ventures: ${d.count} venture(s)${d.count ? ` — ${d.ventures.map((v) => `${v.name} (${v.type}, ${v.status}${v.readyToLaunch ? ', ready to launch' : ''}, quiet cycles ${v.quietCycles}${formatHandledBy(v.handledBy)})`).join(', ')}` : ''}.`;
    ...
    case 'delete-venture':
      // The write-off must never read as a payout. When writtenOffBp > 0 the coin was NOT recovered by
      // anyone — every holder was unpayable, so it simply stopped being tracked along with the deed.
      return `${p}.delete-venture: venture "${d.name}" (${d.ventureId}) removed from the ledger; transaction history retained.${d.writtenOffBp > 0 ? ` ${d.writtenOffBp} BP could not be paid to any holder and was written off — no coin moved to a wallet.` : ''}${d.scrubbedEnterpriseSlots > 0 ? ` ${d.scrubbedEnterpriseSlots} zero-share enterprise ownership slot(s) referencing this deed were scrubbed.` : ''}`;
    case 'launch-venture':
      return `${p}.launch-venture: venture ${d.ventureId} — status now ${d.status}.`;
    case 'wind-up-venture':
      return `${p}.wind-up-venture: venture ${d.ventureId} — status now ${d.status}; the Reckoning table now applies and close-out-venture is reachable.`;
    case 'close-out-venture':
      return `${p}.close-out-venture: venture ${d.ventureId} — status now ${d.status}, working capital released and ${d.distributedBp} BP distributed to holders.`;
    case 'venture-event':
    ...
    case 'cleanup-orphan-ventures':
      return d.outcome === 'noop'
        ? `${p}.cleanup-orphan-ventures: no orphan venture deeds found — nothing removed.`
        : `${p}.cleanup-orphan-ventures: ${d.removedCount} orphan venture deed(s) archived (${d.archiveId}) and removed${d.totalWrittenOffBp > 0 ? `; ${d.totalWrittenOffBp} BP held in escrow could not be paid to anyone and was written off` : ''}.`;
    ...
    case 'trading-check-availability':
      return d.slotCount > 0
        ? `${p}.trading-check-availability: ${d.settlement} (${d.season}) — ${d.slotCount}/${d.potentialSlotCount} slot(s) passed the availability roll: ${d.slots.map((s) => `${s.cargo.name} ${s.amountEp}EP`).join(', ')}.`
        : `${p}.trading-check-availability: ${d.settlement} (${d.season}) — no cargo available (0/${d.potentialSlotCount} slots passed the availability roll).`;
    ...
    case 'trading-sell-cargo':
      return (d.soldPartial
        ...
        + (d.saleType === 'fireSale' ? ' (fire sale — guaranteed half price)' : '')
...
Use this when:
- Standing up a named bank economy for the campaign — accounts, loans, interest, property ownership/rent, and reading its state.
- Moving coin through bank rails (deposit/withdraw/transfer) or adjusting an actor's wallet directly outside a bank account.
- Running the weekly levy/cost-of-living cycle, Money-to-Burn downtime, or the full economic cycle (interest, rentals, venture pass) for an economy.
- Managing a Legitimate Business Enterprise (Archives III) or a Venture Ledger deed — creation, income, debt, ownership shares, lifecycle transitions.
- Running the ported bulk-cargo trading engine (settlements/cargo/pricing/haggling) natively inside this module, or auditing the immutable transaction ledger.

CONFIRM-GATED (CCR-4): delete-economy (archives the economy and dependent active records before removal; transaction history is retained), delete-account (removes the account record; transaction history is kept), money-to-burn (unless dryRun:true), transfer >= 4800 BP (20 GC), resolve-investment, stash-withdraw (both can total-loss the holding on a bad roll), create-enterprise, enterprise-repay-debt, enterprise-upgrade, delete-enterprise, set-enterprise-owners, add-enterprise-debt, forgive-enterprise-debt, save-levy-group, delete-levy-group, create-venture, subscribe-venture, transfer-venture-parts, settle-venture, distribute-venture, delete-venture (permanently removes the deed; transaction history is retained), venture-event (applies a live event band immediately — can move escrow, badge, dilute, or force default), launch-venture, wind-up-venture, close-out-venture (returns working capital to holders), toggle-venture-badge, issue-parts, set-venture-status (a full lifecycle-bypassing GM override; forcing to completed RELEASES trapped capital, forcing to defaulted WRITES IT OFF), set-venture-standing, cleanup-orphan-ventures (permanently archives-then-removes every venture deed with zero resolving handledBy links; any held escrow is written off), and run-economic-cycle (unless dryRun:true) require confirm:true. EXCLUDED (HC-v2-6 dialog-deadlock): advance-day, clearAllFinancialData, and every UI-open — not exposed. RETIRED: banker assignments (Phase 7g); list-bankers remains enum-compatible and returns an empty response marked retired. Also retired (Phase 7d): buy-stock / sell-stock / get-portfolio — the Venture Ledger replaces stock trading; calling any of the three returns a typed WFRP_ECONOMY_ACTION_RETIRED error naming the venture-ledger successors (enum literals stay for HC8 back-compat, but the action itself always fails now). BADGE-GATED (Phase 7d2, D7): subscribe-venture and transfer-venture-parts refuse with WFRP_ECONOMY_VENTURE_DISPUTED while the deed carries the Disputed badge; distribute-venture and settle-venture refuse with WFRP_ECONOMY_VENTURE_SEIZED while Seized. settle-venture also refuses with WFRP_ECONOMY_VENTURE_SETTLE_DELAYED while delayCycles > 0 (a fixed false-success bug, B2).

93 actions:
stand-up-an-economy: list-economies {} · get-economy { economyId, propertyFilter? } · list-bankers { economyId? } (retired compatibility read; always empty) · create-economy { name, currency?, currencySystem?, banks?, properties?, stocks? } · update-economy { economyId, name?, currency?, banks?, properties?, stocks? } · delete-economy { economyId, confirm }.
...
Do NOT use this tool for physical loot/inventory (that's \`module-itempiles\`) or for resolving a character's weekly RAW earnings roll (that's the /wfrp-status skill) — both are explicitly additive to this module (see the intro paragraph above). module-wfrp-economy owns banks/loans/stocks/property/enterprises/ventures/trading and the unified ledger only; it never touches embedded items or the earnings-roll mechanic itself.

Performance Notes:
- Most actions return a small fixed-shape result (an account/enterprise/venture record, a price quote, or a write confirmation) — no response modes, no pagination.
- list-economies/list-accounts/list-transactions/list-enterprises/list-ventures/list-levies/list-levy-groups/trading-list-*: size scales with matching record count in scope; several accept filter params (economyId/actorId/type/etc.) to narrow — pass them rather than requesting the unfiltered set on a large/long-running economy.
- run-economic-cycle/apply-levies/money-to-burn/accrue-interest: response size scales with the number of affected accounts/investments/actors/ventures processed in that pass — still one structured summary per call, not per-entity pagination.`,
        inputSchema: {
          type: 'object',
          properties: {
            ...
            propertyFilter: { type: 'string', enum: ['all', 'owned', 'available'], description: 'get-economy: optional filter over the returned properties array — "owned" (owner is set) / "available" (owner is unset) / omitted or "all" = every property (today\'s unfiltered behavior, back-compat). Matches the wfrp4e-economy fork UI\'s Available/Owned Properties-tab split.' },
            ...
            rolls: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { availabilityRoll: { type: 'number' }, cargoRoll: { type: 'number' }, amountRoll: { type: 'number' } }, required: ['availabilityRoll', 'cargoRoll', 'amountRoll'] }, description: 'trading-check-availability (required): one pre-rolled {availabilityRoll(1-100), cargoRoll(1-100), amountRoll(1-100)} triple per POTENTIAL availability-pipeline slot (Size + 1 if Trade-flagged) — over-supply is fine (extras beyond potentialSlotCount are ignored); under-supply refuses with the required count named. availabilityRoll is the RAW "Availability of Goods" gate (Gazetteer of the Reikland p.75, d100 <= (Size+Wealth)x10) checked INDIVIDUALLY per potential slot — a slot whose availabilityRoll fails contributes no cargo at all, so the market can come back with fewer slots than potentialSlotCount, or none.' },
            ...
            acceptFireSale: { type: 'boolean', description: 'trading-sell-cargo: RAW "Selling" guaranteed fire-sale fallback — only consulted when buyerRoll (and halfCargoRetryRoll, if supplied) BOTH fail. At a Trade-flagged settlement (isTradeSettlement:true), the Characters may always sell at flat half base price instead of walking away; omit/false to keep the pre-existing hard-refusal-on-failure behavior. Refused with gate "fireSale" if isTradeSettlement is false.' },
