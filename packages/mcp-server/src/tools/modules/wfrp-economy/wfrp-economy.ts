...
//
// 25 actions across 8 idioms (stand-up-an-economy, open-a-bank-account, run-a-transaction, loan-cycle,
// investment-cycle, property-management, wallet-quick-adjust, audit-the-ledger). Anchors: DP-15 (concrete
// this.query<WfrpEconomyResult> — never <any>); R2.4 (errors via errorResponse). The transactional writes
// + property/stock ops are LIVE-SMOKE-ONLY (need a configured economy; the smoke-runbook self-bootstraps one).
...
CONFIRM-GATED (CCR-4): delete-economy (purges the economy + its accounts/bankers/portfolios/logs) and transfer >= 4800 BP (20 GC) require confirm:true. EXCLUDED (HC-v2-6 dialog-deadlock): advance-day, clearAllFinancialData, and every UI-open — not exposed.
...
property-management: buy-property { economyId, accountId, propertyId } · sell-property { economyId, accountId, propertyId } · set-rented { economyId, propertyId, rented }.
...
audit-the-ledger: list-transactions { actorId?, economyId?, type?, bankId? } · actor-transaction-summary { actorId, economyId } · bank-transaction-summary { bankId, economyId }.
...
For create-economy, embed a stock to enable the buy/sell round-trip: stocks:[{ name, symbol, currentPrice, availableShares }]. Example: { action: "deposit", economyId: "abc", accountId: "def", amountBp: 240 }`,
        inputSchema: {
          ...
          properties: {
            ...
            accountId: { type: 'string', description: 'Bank account id (deposit / withdraw / loan / stock / property).' },
            ...
            actorId: { type: 'string', description: 'Actor id (create-account / wallet ops / get-portfolio / summaries / list filter).' },
            ...
            amountBp: { type: 'number', description: 'Amount in integer Brass Pennies (deposit/withdraw/transfer/loan/wallet). 1 GC = 240 BP, 1 SS = 12 BP.' },
            ...
            currency: { type: 'string', description: 'create-economy / update-economy: currency display name (default "Gold Crowns").' },
            ...
            type: { type: 'string', description: 'list-transactions: optional transaction-type filter (e.g. deposit, withdraw, stock_purchase).' },
            confirm: { type: 'boolean', description: 'delete-economy / large transfer: must be true to execute the gated op.' },
          },
          required: ['action'],
        },
      },
