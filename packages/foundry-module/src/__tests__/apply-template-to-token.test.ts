...
// verifyDocWrite()s against freshActor._source (both actor writes and the token rename) — the
// stubs must (a) carry a globally-unique uuid registered in `uuidRegistry` and (b) actually
// mutate their own state + a shadow `_source` object on write, mirroring real Foundry semantics.
...
function makeSyntheticActor(opts: { id: string; name: string; type?: string; isToken?: boolean }) {
  ...
  const actor: any = {
    ...
    uuid: `Actor.${opts.id}.${Math.random().toString(36).slice(2, 8)}`,
...
beforeEach(() => {
  ...
  // against the per-test uuidRegistry each makeSyntheticActor() call populates.
  uuidRegistry.clear();
