...
export const MortalNeedsInput = z.discriminatedUnion('action', [
  ...
  z.object({ action: z.literal('get-need-config'), needId: z.string().min(1).optional() }).strict(),
  ...
  z
    .object({
      ...
      entityId: z.string().min(1).optional(), // omitted = party-wide, confirm-gated
  ...
  z
    .object({
      ...
      statusId: z.string().min(1).optional(), // required when consequenceType === 'condition-apply'
